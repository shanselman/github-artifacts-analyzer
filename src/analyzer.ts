...
export class GitHubArtifactsAnalyzer {
  ...
  async analyzeAllRepositories(username?: string, options: AnalysisOptions = { includeExpired: false, minSize: 0 }): Promise<OverallAnalysis> {
  ...
  async analyzeRepository(owner: string, repo: string, options: AnalysisOptions = { includeExpired: false, minSize: 0 }): Promise<RepositoryAnalysis> {
    ...
    try {
      ...
      const artifactsMap = new Map<number, string>(); // workflow run id -> workflow name
      ...
      for (const workflow of analysis.workflows) {
        try {
          ...
          for (const run of runs.workflow_runs) {
            artifactsMap.set(run.id, workflow.name);
